...
from model_skipconnection import *      # TwoDFastGRU, ...
# from model_skipconnection_opt import *      # TwoDFastGRU, ...
...
def train_L(L: int, args, *, warmstart_ckpt_path: str | None):
    ...
    if args.enforce_equal_dims and (args.dmodel != args.dh):
        raise ValueError(
            ...
            f"OR edit model_skipconnection.py to project skip to d_model."
        )
...
def main():
    parser = argparse.ArgumentParser("Iterative retraining with Eq.(6) + staged L=6 + symmetry switch")

    parser.add_argument("--num_layers", type=int, required=True)
    parser.add_argument("--dh", type=int, default=64)
    parser.add_argument("--dmodel", type=int, default=64)
    parser.add_argument("--RNNsymmetry", type=str, default="nosym")
    parser.add_argument("--numsamples", type=int, default=200)
    parser.add_argument("--patch_x", type=int, default=1)
    parser.add_argument("--patch_y", type=int, default=8)
    parser.add_argument("--dotraining", type=str2bool, default=True)

    parser.add_argument("--L_start", type=int, default=6)
    parser.add_argument("--L_end", type=int, default=40)
    parser.add_argument("--L_stride", type=int, default=2)

    parser.add_argument("--scale_s", type=float, default=1.0)
    parser.add_argument("--rate_r", type=float, default=0.25)
    parser.add_argument("--L0", type=int, default=6)
    parser.add_argument("--C", type=float, default=101e3)
    parser.add_argument("--F", type=float, default=2e3)
    parser.add_argument("--min_steps", type=int, default=200)
    parser.add_argument("--round_mode", type=str, default="ceil", choices=["ceil", "round", "floor"])

    parser.add_argument("--lr_stage1", type=float, default=5e-4)
    parser.add_argument("--lr0_stage", type=float, default=5e-4)
    parser.add_argument("--delta0", type=float, default=5e3)
    parser.add_argument("--lr_large", type=float, default=5e-5)
    parser.add_argument(
        "--grad_clip_by_value",
        type=str2bool,
        default=False,
        help="If True, clip gradients elementwise to [-grad_clip_value, +grad_clip_value] before Adam.",
    )
    parser.add_argument(
        "--grad_clip_value",
        type=float,
        default=1.0,
        help="Absolute gradient value used for elementwise clipping when --grad_clip_by_value=True.",
    )

    parser.add_argument("--stage1_base", type=float, default=1e3)
    parser.add_argument("--stage1_scale", type=float, default=50e3)
    parser.add_argument("--stage2_steps", type=float, default=76e3)
    parser.add_argument("--stage3_steps", type=float, default=101e3)

    parser.add_argument("--print_every", type=int, default=200)
    parser.add_argument("--base_dir", type=str, default="./TwoDminGRU_iter")
    parser.add_argument("--resume", action="store_true")
    parser.add_argument("--resume_from_latest", action="store_true",
                        help="With --resume, auto-start from the largest available checkpointed L.")
    parser.add_argument("--ckpt_every", type=int, default=1000,
                        help="Checkpoint interval in training steps (set <=0 to disable periodic saves).")
    parser.add_argument("--seed", type=int, default=1)
    parser.add_argument("--print_schedule", action="store_true")

    # final eval controls
    parser.add_argument("--final_numsamples", type=int, default=10000)
    parser.add_argument("--final_minibatch", type=int, default=2048)
    parser.add_argument("--final_minibatch_min", type=int, default=64)
    parser.add_argument("--final_max_retries_per_batch", type=int, default=10)

    parser.add_argument("--enforce_equal_dims", action="store_true",
                        help="Require dmodel==dh unless you added a skip projection in the model.")

    args = parser.parse_args()
    if args.grad_clip_by_value and args.grad_clip_value <= 0:
        raise ValueError("--grad_clip_value must be > 0 when --grad_clip_by_value=True")

    if args.print_schedule:
        print("\nPlanned steps:")
        n1 = int(args.stage1_base + args.stage1_scale * args.scale_s)
        n2 = int(args.stage2_steps)
        n3 = int(args.stage3_steps)
        print(f"  L={args.L0}: stage1={n1}, stage2={n2}, stage3={n3}")
        for L in range(args.L_start + args.L_stride, args.L_end + 1, args.L_stride):
            nL = nsteps_schedule(L, args.scale_s, args.rate_r, args.L0, args.C, args.F,
                                     args.min_steps, args.round_mode)
            px_eff, py_eff = effective_patch(L, args.patch_x, args.patch_y)
            print(f"  L={L}: steps={nL}, patch=({px_eff},{py_eff})")
        print("")

    loop_start = args.L_start
    first_warmstart = None
    if args.resume and args.resume_from_latest:
        latest = find_latest_checkpoint(args)
        if latest is not None:
            latest_L = latest["L"]
            latest_status = latest["status"]
            print(f"[Resume] latest checkpoint found: L={latest_L}, status={latest_status}")
            if latest_status == "complete":
                loop_start = latest_L + args.L_stride
                first_warmstart = latest["path"]
            else:
                loop_start = latest_L
                first_warmstart = None
        else:
            print("[Resume] no checkpoint found; starting from L_start.")

    if loop_start > args.L_end:
        print("[Resume] nothing to run: all L already completed in requested range.")
        return

    for L in range(loop_start, args.L_end + 1, args.L_stride):
        warmstart = None
        if L == loop_start and first_warmstart is not None:
            warmstart = first_warmstart
        elif L > args.L_start:
            px_prev, py_prev = effective_patch(L - args.L_stride, args.patch_x, args.patch_y)
            prev_path = os.path.join(args.base_dir, f"L_{L-args.L_stride}",
                                     f"dh_{args.dh}", f"numlayers_{args.num_layers}")
            _, warmstart = select_existing_ckpt(
                ckpt_candidates_for_L(L - args.L_stride, args, prev_path, px_prev, py_prev)
            )
        train_L(L, args, warmstart_ckpt_path=warmstart)
